...
$$
\mathbf{y} = \phi\left(\sum_{i=1}^{n} w_i x_i - \theta\right)
$$
...
$$
\mathbf{z} = \mathbf{w}^T \mathbf{x} + b, \quad \mathbf{y} = \phi(\mathbf{z})
$$
...
$$
\tilde{\mathbf{W}} = \begin{bmatrix} \mathbf{W} & \mathbf{b} \end{bmatrix} = \begin{bmatrix} w_{11} & w_{12} & \cdots & w_{1n} & b_1 \\ w_{21} & w_{22} & \cdots & w_{2n} & b_2 \\ \vdots & \vdots & \ddots & \vdots & \vdots \\ w_{m1} & w_{m2} & \cdots & w_{mn} & b_m \end{bmatrix} \in \mathbb{R}^{m \times (n+1)}
$$
...
$$
\mathbf{h}^{(L)} = \mathbf{W}' \mathbf{h}^{(0)} + \mathbf{b}'
$$
...
$$
\mathbf{h}^{(L-1)} = \mathbf{W}_{L-1}' \mathbf{h}^{(0)} + \mathbf{b}_{L-1}'
$$
...
$$
\mathbf{h}^{(L)} = \mathbf{W}^{(L)} \mathbf{h}^{(L-1)} + \mathbf{b}^{(L)} = \mathbf{W}^{(L)} (\mathbf{W}_{L-1}' \mathbf{h}^{(0)} + \mathbf{b}_{L-1}') + \mathbf{b}^{(L)}
$$
$$= (\mathbf{W}^{(L)} \mathbf{W}_{L-1}') \mathbf{h}^{(0)} + (\mathbf{W}^{(L)} \mathbf{b}_{L-1}' + \mathbf{b}^{(L)})
$$
这正是$L$层网络的等效仿射变换形式。
...
（4）GELU激活函数：
$$
\text{GELU}(z) = z \cdot \Phi(z) = \frac{z}{2} \left[1 + \text{erf}\left(\frac{z}{\sqrt{2}}\right)\right]
$$
...
$$
{x∣wTx+b=0}\{\mathbf{x} \mid \mathbf{w}^T \mathbf{x} + b = 0\}
$$
...
$$
P(y = 1 \mid \mathbf{x}) = p, \quad P(y = 0 \mid \mathbf{x}) = 1 - p
$$
...
$$
\sigma(\mathbf{z}) = \frac{1}{1 + e^{-\mathbf{z}}} = \frac{e^{\mathbf{z}}}{e^{\mathbf{z}} + 1}​
$$
...
$$
P(y = k \mid \mathbf{x}) = \frac{e^{z_k}}{\sum_{j=1}^{K} e^{z_j}}, \quad \text{where } \mathbf{z} = \mathbf{W}\mathbf{x} + \mathbf{b}
$$
...
证明：对于性质（1），直接计算：
$$
\sum_{k=1}^{K} \frac{e^{z_k}}{\sum_{j=1}^{K} e^{z_j}} = \frac{\sum_{k=1}^{K} e^{z_k}}{\sum_{j=1}^{K} e^{z_j}} = 1∑k=1
$$

对于性质（2）：
$$
\text{softmax}(\mathbf{z} + c\mathbf{1})_k = \frac{e^{z_k + c}}{\sum_{j=1}^{K} e^{z_j + c}} = \frac{e^c e^{z_k}}{e^c \sum_{j=1}^{K} e^{z_j}} = \frac{e^{z_k}}{\sum_{j=1}^{K} e^{z_j}} = \text{softmax}(\mathbf{z})_k​
$$
对于性质（3），考虑比值：
$$
\frac{\text{softmax}(\mathbf{z})_i}{\text{softmax}(\mathbf{z})_j} = \frac{e^{z_i} / \sum e^{z_k}}{e^{z_j} / \sum e^{z_k}} = e^{z_i - z_j} > 1 \quad \text{if } z_i > z_j​
$$
...
$$
\mathbf{Z} = \mathbf{X}\mathbf{W} + \mathbf{1}_N \mathbf{b}^T, \quad \mathbf{Y} = \phi(\mathbf{Z})
$$
...
$$
\mathbf{z}_i = \mathbf{x}_i \mathbf{W} + \mathbf{b}^T = \sum_{k=1}^{d} x_{ik} \mathbf{w}_k + \mathbf{b}^T
$$
