...
$$
f^{(\ell)}(\mathbf{h}) = \phi^{(\ell)}\left(\mathbf{W}^{(\ell)}\mathbf{h} + \mathbf{b}^{(\ell)}\right)
$$
...
$$
f_{\Theta} = f^{(L)} \circ f^{(L-1)} \circ \cdots \circ f^{(1)}
$$
对于任意输入$\mathbf{x}$，有：
$$
f_{\Theta}(\mathbf{x}) = f^{(L)}\left(f^{(L-1)}\left(\cdots f^{(1)}(\mathbf{x}) \cdots\right)\right)
$$
...
或等价地，使用莱布尼茨记号表示为：
$$
\frac{dy}{dx} = \frac{dy}{dz} \cdot \frac{dz}{dx}, \quad \text{其中 } z = g(x), y = f(z)
$$
...
$$
f(g(x + \Delta x)) - f(g(x)) = f'(\xi) \cdot (g(x + \Delta x) - g(x))
$$
当$\Delta x \to 0$时，$\xi \to g(x)$，故：
$$
\lim_{\Delta x \to 0} \frac{f(g(x + \Delta x)) - f(g(x))}{\Delta x} = f'(g(x)) \cdot g'(x)
$$
...
$$
\frac{\partial (\mathbf{y})_i}{\partial (\mathbf{x})_j} = \sum_k \frac{\partial f_i}{\partial z_k} \cdot \frac{\partial z_k}{\partial x_j}​​
$$
...
$$
h_i = \begin{cases} z_i & \text{if } s_i = 1 \\ 0 & \text{if } s_i = -1 \end{cases}​
$$
...
$$
\text{FLOPs}_{\text{total}} = \sum_{\ell=1}^{L} B \cdot (2n_{\ell-1}n_\ell + n_\ell) = O\left(B \sum_{\ell=1}^{L} n_{\ell-1}n_\ell\right)
$$
