...
$$E_i(\mathbf{x}) = \sigma(\mathbf{x}^\top \mathbf{W}_1^{(i)} + \mathbf{b}_1^{(i)}) \mathbf{W}_2^{(i)} + \mathbf{b}_2^{(i)})\tag{7.2.1}$$
...
$$\mathbf{h}_i = \sigma(\mathbf{W}_1^{(i)} \mathbf{x} + \mathbf{b}_1^{(i)})\tag{7.2.2}$$
$$E_i(\mathbf{x}) = \mathbf{W}_2^{(i)} \mathbf{h}_i + \mathbf{b}_2^{(i)})\tag{7.2.3}$$
...
$$\mathcal{E}(\mathbf{x}) = (E_1(\mathbf{x}), E_2(\mathbf{x}), \ldots, E_K(\mathbf{x}))\tag{7.2.4}$$
...
$$E_i^{\text{shared}}(\mathbf{x}) = \sigma(\mathbf{x}^\top \mathbf{W}_1^{\text{shared}} + \mathbf{b}_1^{\text{shared}}) \mathbf{W}_2^{\text{shared}} + \mathbf{b}_2^{\text{shared}}\tag{7.2.5}$$
$$E_i^{\text{unique}}(\mathbf{x}) = \sigma(\mathbf{x}^\top \mathbf{W}_1^{(i)} + \mathbf{b}_1^{(i)}) \mathbf{W}_2^{(i)} + \mathbf{b}_2^{(i)})\tag{7.2.6}$$
$$E_i(\mathbf{x}) = E_i^{\text{shared}}(\mathbf{x}) + E_i^{\text{unique}}(\mathbf{x})\tag{7.2.7}$$
...
$$\mathbf{H}_{\text{MoE}} = \text{MoE-Layer}(\mathbf{H}_{\text{attn}}) = \sum_{i=1}^{K} \mathbf{G}_i \odot E_i(\mathbf{H}_{\text{attn}})\tag{7.2.8}$$
...
$$\mathbf{y} = \sum_{i=1}^{K} g_i(\mathbf{x}) \cdot E_i(\mathbf{x})\tag{7.2.9}$$
...
$$\mathbf{y} = \sum_{i \in S(\mathbf{x})} g_i^{\text{sparse}}(\mathbf{x}) \cdot E_i(\mathbf{x})\tag{7.2.10}$$
其中稀疏门控权重 $g_i^{\text{sparse}}(\mathbf{x})$ 满足归一化条件 $\sum_{i \in S(\mathbf{x})} g_i^{\text{sparse}}(\mathbf{x}) = 1$。这种归一化通过在激活专家的权重上重新应用Softmax实现：
$$g_i^{\text{sparse}}(\mathbf{x}) = \frac{\exp(s_i(\mathbf{x}))}{\sum_{j \in S(\mathbf{x})} \exp(s_j(\mathbf{x}))}, \quad i \in S(\mathbf{x})\tag{7.2.11}$$
...
$$\mathbf{y} = \left(\sum_{i=1}^{K} g_i(\mathbf{x}) \cdot \mathbf{W}_2^{(i)} \cdot \sigma(\mathbf{W}_1^{(i)} \mathbf{x} + \mathbf{b}_1^{(i)})\right) + \sum_{i=1}^{K} g_i(\mathbf{x}) \cdot \mathbf{b}_2^{(i)}\tag{7.2.12}$$
...
$$\mathbf{Y}_{b,s,:} = \sum_{i=1}^{K} \mathbf{G}_{b,s,i} \cdot \mathbf{E}_i(\mathbf{X})_{b,s,:}\tag{7.2.13}$$
...
$$\frac{\partial \mathcal{L}}{\partial g_i(\mathbf{x})} = \frac{\partial \mathcal{L}}{\partial \mathbf{y}} \cdot E_i(\mathbf{x})\tag{7.2.14}$$
$$\frac{\partial \mathcal{L}}{\partial \theta_i} = \frac{\partial \mathcal{L}}{\partial g_i(\mathbf{x})} \cdot \frac{\partial g_i(\mathbf{x})}{\partial \theta_i} + \frac{\partial \mathcal{L}}{\partial \mathbf{y}} \cdot \frac{\partial E_i(\mathbf{x})}{\partial \theta_i} \cdot \mathbb{I}(i \in S(\mathbf{x}))\tag{7.2.15}$$
...
$$p_i(\mathbf{x}) = \frac{\exp(s_i(\mathbf{x}))}{\sum_{j=1}^{K} \exp(s_j(\mathbf{x}))}\tag{7.2.16}$$
...
$$\begin{align*}
& P_i = \mathbb{P}(I_i = 1) = \\& \mathbb{E}_{\mathbf{x} \sim p_{\text{data}}} \left[ \mathbb{I}(p_i(\mathbf{x}) \text{ 属于最高的 } K' \text{ 个概率}) \right]\tag{7.2.17}
\end{align*}
$$
...
$$\text{Var}(P) = \frac{1}{K} \sum_{i=1}^{K} (P_i - \bar{P})^2\tag{7.2.18}$$
...
$$P(I_j = 1 | I_i = 1) = \frac{\mathbb{P}(I_i = 1, I_j = 1)}{P_i}\tag{7.2.19}$$
...
$$f_i = \frac{1}{|\mathcal{B}|} \sum_{\mathbf{x} \in \mathcal{B}} \mathbb{I}(i \in S(\mathbf{x}))\tag{7.2.20}$$
...
$$\mathcal{L}_{\text{balance}} = -\sum_{i=1}^{K} \frac{1}{K} \log f_i = \log K - \frac{1}{K} \sum_{i=1}^{K} \log f_i\tag{7.2.21}$$
这个损失在 $f_i = 1/K$ 时最小（值为0），当 $f_i$ 偏离均匀分布时增大。然而，这个损失存在问题：当 $f_i = 0$ 时，$\log f_i$ 趋向负无穷，导致损失爆炸。因此，实际中通常使用更稳定的变体：
$$\mathcal{L}_{\text{balance}} = K \cdot \sum_{i=1}^{K} f_i \cdot \log(K \cdot f_i)\tag{7.2.22}$$
...
另一种常用的负载均衡损失是平方偏差损失：
$$\mathcal{L}_{\text{balance}}^{\text{squared}} = \sum_{i=1}^{K} \left( f_i - \frac{1}{K} \right)^2\tag{7.2.23}$$
...
$$\mathcal{L}_{\text{balance}} = \sum_{i=1}^{K} f_i \cdot P_i\tag{7.2.24}$$
...
$$\sum_{i=1}^{K} f_i P_i \geq \frac{1}{K} \sum_{i=1}^{K} P_i = \frac{K'}{K}\tag{7.2.25}$$
当 $f_i$ 与 $P_i$ 成正比时等号成立，这表明在给定 $P_i$ 的情况下，$f_i$ 应该与 $P_i$ 匹配以最小化损失。
在实际训练中，负载均衡损失通常乘以一个权重系数 $\lambda$ 后与主损失（如交叉熵损失）相加：
$$\mathcal{L}_{\text{total}} = \mathcal{L}_{\text{main}} + \lambda \cdot \mathcal{L}_{\text{balance}}\tag{7.2.26}$$
...
$$\frac{\partial \mathcal{L}_{\text{balance}}}{\partial P_i} = f_i\tag{7.2.28}$$
...
路由概率的方差定义为：
$$\text{Var}(P) = \frac{1}{K} \sum_{i=1}^{K} \left(P_i - \bar{P}\right)^2\tag{7.2.29}$$
...
$$\mathcal{L}_{\text{importance}} = \text{Var}(P) = \frac{1}{K} \sum_{i=1}^{K} \left(P_i - \frac{K'}{K}\right)^2\tag{7.2.30}$$
这个损失在 $P_i = K'/K$ 对所有 $i$ 成立时最小（值为0），此时所有专家具有相等的平均路由概率，负载完全均衡。
从优化的角度看，Importance Weight损失关于门控网络参数 $\mathbf{W}$ 和 $\mathbf{b}$ 的梯度可以表示为：
$$\frac{\partial \mathcal{L}_{\text{importance}}}{\partial \mathbf{W}} = \frac{2}{K} \sum_{i=1}^{K} \left(P_i - \frac{K'}{K}\right) \cdot \frac{\partial P_i}{\partial \mathbf{W}}\tag{7.2.31}$$
...
$$\mathcal{L}_{\text{importance}}^{\text{margin}} = \frac{1}{K} \sum_{i=1}^{K} \max\left(0, P_i - \frac{K'}{K} - m\right)^2\tag{7.2.32}$$
...
本节系统阐述了MoE架构中专家网络的数学表示与负载均衡的理论基础，从多个维度深入分析了条件计算范式下的核心挑战与解决方案。专家网络作为MoE架构中执行具体计算任务的功能模块，其设计直接影响模型的整体表达能力和计算效率。每个专家采用两层前馈神经网络的形式，通过连续的线性变换和非线性激活函数实现从输入空间到输出空间的函数映射。专家集合可数学表示为将输入向量映射为多个专家输出集合的映射函数，强调了专家之间的并行计算特性。加权组合输出则通过门控权重的线性组合将专家计算结果融合，体现了条件计算范式的核心理念，同时为梯度传播和参数优化提供了清晰的数学框架。

路由概率与专家分配的分析揭示了门控机制的核心机制与潜在问题。路由概率通过Softmax函数归一化实现，确保概率分布的基本性质得以满足。Top-K稀疏路由策略使得每个输入仅激活少数专家，在保持表达能力的同時降低计算成本。专家分配概率的统计特性，包括方差、条件期望和时间动态，为理解训练过程中的路由行为提供了定量工具。负载均衡辅助损失函数的设计是解决训练挑战的关键，通过交叉熵形式或平方偏差形式的正则化，强制门控网络促进专家的均匀使用。Importance Weight机制从统计方差的角度量化负载不均衡，通过梯度下降最小化路由概率的方差，实现更精细的负载均衡控制。

防止专家坍缩是MoE训练中的核心挑战，其数学本质是门控网络陷入不良局部最优导致路由决策极度不均衡。专家坍缩的发生可用梯度消失机制解释：未被激活的专家无法接收梯度来调整参数，形成正反馈循环加剧不均衡。负载均衡损失通过双重正则化机制打破这一循环，推动路由概率向实际分配频率方向调整，防止专家容量的浪费。多种辅助策略包括噪声注入、专家容量限制和辅助专家的引入，从不同角度完善了防止坍缩的技术体系。本节的内容为理解和实现高效的MoE系统提供了坚实的理论基础，对于在大规模训练场景中充分发挥MoE架构的潜力具有重要指导意义。